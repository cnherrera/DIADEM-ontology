...
BASE = Namespace("https://cnherrera.github.io/DIADEM-ontology/proj#") # instance base
...
def main():
    ...
    if args.project:
        ...
        out_path = Path(args.output) if args.output else Path(f"{slug(acronym)}_instances.ttl")
        ...
        out_path = Path(args.output) if args.output else Path(
            f"{slug(prog_data['program']['acronym'])}_instances.ttl"
        )
        g.serialize(destination=str(out_path), format="turtle")
        print(f"✔  Written {out_path}  ({len(g)} triples)")
